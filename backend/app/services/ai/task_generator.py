...
async def generate_task_details(
    ...
    business_context = await _get_business_context(tenant_id)
...
async def _get_business_context(tenant_id: str) -> str:
    """Get business profile context for better AI suggestions."""
    profile = await BusinessProfile.find_one(BusinessProfile.tenant_id == tenant_id)
